...
ENV_NAME = 'CartPole'
APP_NAME = ENV_NAME.lower() + '-ddqn-pytorch'
...
def build_model(env, n_hidden):
    n_in = env.observation_space.shape[0]
    n_out = env.action_space.n
    return Model(n_in, n_out, n_hidden).to(device)
...
if '__main__' == __name__:
    ...
    model = build_model(env, 256)
    target_model = build_model(env, 256)
    opt = t.optim.Adam(model.parameters())
    loss_fn = t.nn.MSELoss()

    eps, eps_final = 1, 0.05
    eps_steps = 1000
    gamma = 0.99
    replay = Replay(10000)
    batch_size = 32
    ...
    for episode in episodes_tqdm:
        ...
        while not done:
            ...
            if len(replay) > 1000:
                loss = update(model, target_model, opt, loss_fn, replay, batch_size)
                losses.append(loss)
