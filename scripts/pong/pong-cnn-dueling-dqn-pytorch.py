...
APP_NAME = ENV_NAME.lower() + '-cnn-dueling-dqn-pytorch'
...
def parse_args():
    ...
    parser.add_argument('--not-double', action='store_false', dest='double',
                        help='no not use separate target model')
...
class Model(t.nn.Module):
    ...
    def forward(self, x):
        ...
        adv = self.adv(p)
        val = self.val(p)
        adv_mean = adv.mean(1).unsqueeze(1)
        return adv - adv_mean + val
...
if '__main__' == __name__:
    ...
    target_model = build_model(frames_per_state).to(device) if args.double else model
    ...
    for episode in episodes_tqdm:
        ...
        while not done:
            ...
            if len(replay) > batch_size * args.steps and frame % args.steps == 0:
                ...
                if args.double and frame % 1000 == 0:
